module Main
    ...
    let runIterations func iterations count =
        let reducer acc elem =
            ...
            for i in 0 .. count do
                func i |> ignore
            let spentTime = DateTime.UtcNow.Subtract(started).TotalMilliseconds * 1000000.0
            let time = spentTime / float count
            let deltaTime, deltaTimeSquared, runningTime = acc
            deltaTime + time, deltaTimeSquared + (time * time), spentTime
